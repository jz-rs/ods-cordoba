...
const links = [
  { anchor: "observatorio",     label: "Observatorio" },
  { anchor: "ods-cordoba-info", label: "Sobre los ODS" },
  { anchor: "ods-cordoba",      label: "Córdoba" },
  { anchor: "sdg-grid",         label: "Los 17 objetivos" },
  { label: "Aprende", external: "https://juegosods.com/" },
  { anchor: "participar",       label: "Sumate", highlight: true },
];
...
export default function Navbar() {
  ...
  // En la home: ancla directa. En otra página: navegar a la home con el ancla.
  const anchorHref = (anchor?: string) =>
    anchor ? (isHome ? `#${anchor}` : `/#${anchor}`) : undefined;
  const close = () => setOpen(false);

  // El scroll suave solo tiene sentido si ya estamos en la home;
  // en otras páginas el Link navega normalmente.
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:"60px"}}>
        ...
        <nav className="nav-desktop" style={{display:"flex",alignItems:"center",gap:"2px"}}>
          {links.map((l) => {
            ...
            if (l.highlight) {
              return (
                <a key={l.label} href={anchorHref(l.anchor)}
            ...
            return (
              <a key={l.label} href={anchorHref(l.anchor)} className="nav-link"
                 aria-label={`Ir a la sección ${l.label}`}
                 style={{color:"#475569",fontSize:"13px",fontWeight:600,padding:"7px 14px",borderRadius:"8px",textDecoration:"none",letterSpacing:"0.01em"}}>
                {l.label}
              </a>
